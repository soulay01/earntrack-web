...
export default function SettingsPage() {
  ...
  const navCards = [
    { href: '/settings/firmendaten', label: 'Firmendaten', desc: 'Adresse, Steuernummer & Bankdaten', icon: <Building2 className="w-6 h-6 text-rose-600" />, gradient: 'from-rose-50 to-pink-50', border: 'border-rose-200' },
        ...
        <div className="px-4 md:px-8 py-4 md:py-8 max-w-2xl mx-auto space-y-8">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-lg transition-all duration-300 p-6 ">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div>
                <p className="text-slate-900 font-bold text-lg">{company?.name || 'Meine Firma'}</p>
                <p className="text-slate-400 text-sm">{user.email}</p>
              </div>
            </div>
            <div className="h-px bg-slate-100 mb-4" />
            <div className="mb-4">
              <p className="text-slate-400 text-xs font-semibold mb-1.5">Dein Name <span className="font-normal">(für Begrüßung &amp; Messenger)</span></p>
              <div className="flex gap-2">
                <input
                  ...
                  maxLength={60}
                  className="flex-1 px-4 py-2.5 rounded-xl border border-slate-200 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-teal-500/30 focus:border-teal-400 transition-all"
                />
                <button
                  disabled={savingName || !nameDraft.trim() || nameDraft.trim() === (userName || '')}
                  onClick={async () => {
                    setSavingName(true);
                    try {
                  ...
                  className="px-4 py-2.5 rounded-xl bg-teal-600 hover:bg-teal-700 text-white text-sm font-bold transition-all active:scale-[0.97] disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1.5"
                >
                  {nameSaved ? <Check className="w-4 h-4" /> : null}
                  {savingName ? 'Speichert…' : nameSaved ? 'Gespeichert' : 'Speichern'}
                </button>
              </div>
            </div>
