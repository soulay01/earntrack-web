...
import { loginEmail, loginGoogle, loginApple, resetPw } from '@/lib/auth';
...
function LoginInner() {
  ...
  useEffect(() => {
    ...
    // Warten, bis das Firmen-Dokument tatsächlich geladen ist (nicht nur "loading"
    // aus dem Auth-Teil) — sonst ist `company` beim ersten Rendern noch null und
    // ein frisches Google/Apple-Konto würde fälschlich sofort ins Dashboard springen,
    // bevor das needsOnboarding-Flag ankommt. Employees haben kein Company-Listening,
    // daher nur für Owner abwarten.
    if (role === 'owner' && !companyLoaded) return;
    // Neuer Google/Apple-Login ohne bestehendes Konto → erst Profil vervollständigen
    // (Firma/Telefon/Adresse), statt direkt in die App zu lassen.
    router.push(company?.needsOnboarding ? '/demo' : redirect);
  }, [user, loading, role, companyLoaded, company, router, redirect]);
          ...
          {reg ? (
              ...
              <div className="space-y-4">
                <div className="bg-gradient-to-br from-violet-50 to-fuchsia-50 border border-violet-200 rounded-2xl p-5 space-y-3 animate-stagger-1">
                  <div className="flex items-start gap-3">
                    <span className="w-6 h-6 rounded-full bg-gradient-to-br from-violet-500 to-fuchsia-500 flex items-center justify-center text-white shrink-0 mt-0.5"><Check className="w-3.5 h-3.5" /></span>
            ...
            <div className="p-7">
              ...
              <div className="mt-5 pt-5 border-t border-slate-100 animate-stagger-4 opacity-0">
                ...
                <button
                  type="button" onClick={apple}
                  className="w-full py-3 bg-black hover:bg-slate-800 active:scale-[0.98] text-white font-semibold rounded-2xl transition-all text-sm flex items-center justify-center gap-3 mt-3"
                >
