...
export default function DashboardCalendar({ value, onChange, onClose }: { value: string; onChange: (d: string) => void; onClose: () => void }) {
  const today = new Date();
  const parsed = parseDateString(value) || today;
  const [year, setYear] = useState(parsed.getFullYear());
  const [month, setMonth] = useState(parsed.getMonth());
  const holidays = getGermanHolidays(year, month);
    ...
    <>
      <div className="flex items-center justify-between mb-3">
        <button type="button" onClick={() => { if (month === 0) { setYear(y => y - 1); setMonth(11); } else setMonth(m => m - 1); }}
          className="p-2 rounded-lg hover:bg-slate-100 text-slate-500 active:scale-[0.9] transition-all">
          ...
          const isToday = d !== null && formatDate(new Date()) === formatDate(new Date(year, month, d));
            ...
            return (
              <button key={i} type="button" onClick={() => select(d)} title={holidayName || undefined}
                className={`text-xs font-semibold w-9 h-9 rounded-lg flex flex-col items-center justify-center transition-all active:scale-[0.9] relative ${
      ...
      <button type="button" onClick={() => select(today.getDate())}
        className="mt-3 w-full py-2.5 rounded-lg text-xs font-semibold text-teal-600 hover:bg-teal-50 transition-all active:scale-[0.97]">
        Heute
      </button>
    </>
  );
