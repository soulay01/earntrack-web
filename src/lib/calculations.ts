...
// Score fürs Angebot: rein margin-basiert, identisch zur Note eines fertigen
// Einsatzes (calculateAssignmentProfitScore in smartPricing.ts) – 40 % Marge ergibt
// bei Angebot UND Einsatz dieselbe Note, unabhängig von Auftragsgröße oder Anzahl
// ausgefüllter Felder. Vorher floss Angebotsgröße (30 %) und Formular-Vollständigkeit
// (20 %) direkt in den Score ein, wodurch ein kleines 40%-Margen-Angebot schlechter
// abschnitt als derselbe Auftrag nach Abschluss. Vollständigkeit bleibt als reiner
// Warnhinweis (dataComplete/missingFields, analog zum Einsatz) erhalten, verändert
// die Note aber nicht mehr.
export function calculateEstimateProfitScore(input: {
  ...
  const score = Math.round(Math.max(0, Math.min(100, marginPoints * 1.5)));
  ...
  const missingFields = checkList.filter(c => !c.met).map(c => c.label);
